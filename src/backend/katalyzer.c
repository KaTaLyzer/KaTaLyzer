...
static void parse_application_layer(char *protokol)
{
  ...
  for(j=0;j<i_TCP; j++) {
    ...
    if(s_protocol == pole_TCP_portov[j] || d_protocol == pole_TCP_portov[j]) {                      
      if(s_protocol == pole_TCP_portov[j])
        sprintf(protokol,"TCP%d",s_protocol);
      if(d_protocol == pole_TCP_portov[j])
        sprintf(protokol,"TCP%d",d_protocol);
      m_protokoly(&z_protokoly,protokol);
    }
  }
  ...
  for(j=0;j<i_UDP; j++){
    if(s_protocol == pole_UDP_portov[j] || d_protocol == pole_UDP_portov[j]) {
      if(s_protocol == pole_UDP_portov[j])
        sprintf(protokol,"UDP%d",s_protocol);
      if(d_protocol == pole_UDP_portov[j])
        sprintf(protokol,"UDP%d",d_protocol);
      m_protokoly(&z_protokoly,protokol);
    }
  }
}
...
#ifdef CDP_P
static void cdp_function(MYSQL *conn)
{
  char prikaz[20000];
...
#ifdef SNMP_P
static void snmp_function(MYSQL *conn)
{
  char prikaz[20000];
      ...
      *arg = casy[i];
      switch(pthread_create(&cron,NULL,cronovanie,(void *) arg)) {
        case -1:
          ...
          break;
        case 0:
          fprintf(stderr,"cronovanie(%d) [DONE]\n",casy[i]);
          break;
        default: break;
      }
      flow_flag=0;
...
////////////////////////////////////////////////////////////////////////
 ...
 if((flag == 0 && (actual_time - beggining_time) >= casovac_zapisu) ||
   ((actual_time - beggining_time) >= casovac_zapisu*2)) { // flag - were data written into DB in this time interval? 
   
   database(head_dt, header);
...
void m_protokoly(ZACIATOK_P *p_zac, char *s) {
  ...
  if(p_zac->empty) { //if struct is empty, we create it
    ...
    if(!find_p) { // we don't find the protocol, so we crete new structure for the new protocol
      for(help_protokol=p_zac->p_protokoly;help_protokol->p_next!=NULL;help_protokol=help_protokol->p_next)
        ; //dojde na koniec struktury
